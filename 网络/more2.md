...
//如果用户release了一次，那么什么都不做，因为单例模式在整个程序运行过程中都拥有且只有一份，
  程序退出之后被释放，所以不需要对引用计数器操作
-(oneway void)release
{
}
...
答：单例是不可以用继承的，如果继承了地址都一样，继承也没有意义了；如果想一次写就，四处使用，那么推荐亲使用带参数的宏定义啦！
04 问：宏定义怎么弄？
答：这个嘛~~回头看一眼我的代码咯，亲。
```
...
### 2.NSOperation
> 01 NSOperation本身是抽象类，是对GCD的包装；，只能由它的子类封装任务
...
### 3.多图下载综合示例程序
![](/assets/屏幕快照 2560-02-07 12.03.45.png)
（1）涉及知识点
