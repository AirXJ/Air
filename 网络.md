...
（1）GCD基本知识[^队列存放任务调度任务,一个获取内存里的并发队列，一个是自己创建][^异步可以在新的线程中执行任务,具备开启线程的能力(说的意思是可以开也可以不开，但是肯定会开一条，不然同串并串没区别了，所以具体开几条看系统安排)][^同步异步，手里的活能不能待会空的时候做;主线程有个同步任务要求你放到主队列里完成，手里有活又要干别的死锁]
    01 两个核心概念-队列和任务，任务的取出遵循队列的FIFO原则
    02 同步函数和异步函数[^同步只能在当前线程中执行任务,不具备开启新线程的能力]

（2）GCD基本使用【重点】
1创建队列 2.(选择同步异步)封装任务，添加任务到队列
```objc
